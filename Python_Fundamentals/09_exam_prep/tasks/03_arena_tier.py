...
while True:
    ...
    if len(splitted_data) == 3:
        ...
        fighter_1 = line.split(' vs ')[0]
        fighter_2 = line.split(' vs ')[1]

        if fighter_1 in gladiators_names_list and fighter_2 in gladiators_names_list:
            fighter_1_techniques_list = list(filter(lambda gl: gl.gladiator_name == fighter_1, gladiators_list))[
                0].skills.keys()
            fighter_2_techniques_list = list(filter(lambda gl: gl.gladiator_name == fighter_2, gladiators_list))[
                0].skills.keys()
            common_skills = list(set(fighter_1_techniques_list).intersection(set(fighter_2_techniques_list)))
...
for gladiator in ordered_gladiators:
    ...
    for technique, skill in sorted(gladiator.skills.items(), key=lambda kvp: kvp[1], reverse=True):
